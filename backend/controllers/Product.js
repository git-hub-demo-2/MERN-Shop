...
exports.create=async(req,res)=>{
    try {
        const { title, brand, category, description, price, discountPercentage, stockQuantity } = req.body;
        const thumbnail = req.files["thumbnail"] ? `/storage/${req.files["thumbnail"][0].filename}` : null;
        const images = req.files["images"] ? req.files["images"].map(file => `/storage/${file.filename}`) : [];
